...
function load_all(datadir, nqubit)
    optimizers = ["base", "improved", "coverage"]
    all_results = Dict(
...
const OPTIMIZERS = [
    ("base",     Makie.wong_colors()[1]),
    ("improved", Makie.wong_colors()[2]),
    ("coverage", Makie.wong_colors()[3]),
]
...
function plot_delta_heatmaps(results, outdir)
    ...
    comparisons = [("improved", Makie.wong_colors()[2]),
                   ("coverage", Makie.wong_colors()[3])]
