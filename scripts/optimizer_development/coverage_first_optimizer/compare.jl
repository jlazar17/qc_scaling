...
function run_comparison()
    ...
    mkpath(args["outdir"])
    outfiles = Dict(
        "base"     => joinpath(args["outdir"], "base_nqubit_$(nqubit).h5"),
        "improved" => joinpath(args["outdir"], "improved_nqubit_$(nqubit).h5"),
        "coverage" => joinpath(args["outdir"], "coverage_nqubit_$(nqubit).h5"),
    )
    optimizers = Dict(
        "base"     => base_optimization,
        "improved" => improved_optimization,
        "coverage" => coverage_optimization,
    )
        ...
        for pzero in pzero_values
            for seed in seeds
                ...
                for (name, optfn) in optimizers
                    ...
                    run_args = merge(common, Dict(
                        "outfile"  => outfiles[name],
                        "p_mutate" => args["p-mutate"],
                    ))
                    optfn(make_goal, run_args)
                end
            end
        end
    end
